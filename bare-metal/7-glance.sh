...
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'localhost' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'%' IDENTIFIED BY 'devstack';
_EOF_

source admin-openrc

openstack user create --domain default --password devstack glance >> glance.log 2>> glance-error.log
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/glance\/glance.sqlite/mysql+pymysql:\/\/glance:devstack@controller\/glance/' /etc/glance/glance-api.conf
...
sudo sed -i "$[linhaauthtoken+9] i\password = devstack" /etc/glance/glance-api.conf
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/glance\/glance.sqlite/mysql+pymysql:\/\/glance:devstack@controller\/glance/' /etc/glance/glance-registry.conf
...
sudo sed -i "$[linhaauthtoken2+9] i\password = devstack" /etc/glance/glance-registry.conf
