...
GRANT ALL PRIVILEGES ON nova_api.* TO 'nova'@'localhost' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON nova_api.* TO 'nova'@'%' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'localhost' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'%' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'localhost' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'%' IDENTIFIED BY 'devstack';
_EOF_

source ../admin-openrc

openstack user create --domain default --password devstack nova >> nova.log 2>> nova-error.log
...
openstack user create --domain default --password devstack placement >> nova.log 2>> nova-error.log
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/nova\/nova_api.sqlite/mysql+pymysql:\/\/nova:devstack@controller\/nova_api/' /etc/nova/nova.conf
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/nova\/nova.sqlite/mysql+pymysql:\/\/nova:devstack@controller\/nova/' /etc/nova/nova.conf

linhadefaultnova=`sudo awk '{if ($0 == "[DEFAULT]") {print NR;}}' /etc/nova/nova.conf`
sudo sed -i "$[linhadefaultnova+4] i\transport_url = rabbit://openstack:devstack@controller" /etc/nova/nova.conf
sudo sed -i "$[linhadefaultnova+5] i\my_ip = 10.0.0.17" /etc/nova/nova.conf
...
sudo sed -i "$[linhaauthtokennova+8] i\password = devstack" /etc/nova/nova.conf

linhavncnova=`sudo awk '{if ($0 == "[vnc]") {print NR;}}' /etc/nova/nova.conf`
sudo sed -i "$[linhavncnova+1] i\enabled = True" /etc/nova/nova.conf
sudo sed -i "$[linhavncnova+2] i\server_listen = 10.0.0.17" /etc/nova/nova.conf
sudo sed -i "$[linhavncnova+3] i\server_proxyclient_address = 10.0.0.17" /etc/nova/nova.conf
...
sudo sed -i "$[linhaplacementnova+8] i\password = devstack" /etc/nova/nova.conf
