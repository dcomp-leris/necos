...
GRANT ALL PRIVILEGES ON neutron.* TO 'neutron'@'localhost' IDENTIFIED BY 'devstack';
GRANT ALL PRIVILEGES ON neutron.* TO 'neutron'@'%' IDENTIFIED BY 'devstack';
_EOF_

source ../admin-openrc

openstack user create --domain default --password devstack neutron >> neutron.log 2>> neutron-error.log
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/neutron\/neutron.sqlite/mysql+pymysql:\/\/neutron:devstack@controller\/neutron/' /etc/neutron/neutron.conf
...
sudo sed -i "$[linhadefaultneutron+4] i\transport_url = rabbit://openstack:devstack@controller" /etc/neutron/neutron.conf
...
sudo sed -i "$[linhanovaneutron+8] i\password = devstack" /etc/neutron/neutron.conf
...
sudo sed -i "$[linhavxlanneutron+2] i\local_ip = 10.0.0.17" /etc/neutron/plugins/ml2/linuxbridge_agent.ini
...
sudo sed -i "$[linhadefaultmetadataneutron+2] i\metadata_proxy_shared_secret = devstack" /etc/neutron/metadata_agent.ini
...
sudo sed -i "$[linhaneutronnova+11] i\metadata_proxy_shared_secret = devstack" /etc/nova/nova.conf
