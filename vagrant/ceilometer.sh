...
sudo sed -i "$[linhaDefault+2] i\transport_url = rabbit://openstack:$1@controller" /etc/ceilometer/ceilometer.conf
...
sudo sed -i "$[linhaCredentials+7] i\password = $1" /etc/ceilometer/ceilometer.conf
