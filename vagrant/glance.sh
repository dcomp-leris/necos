...
openstack user create --domain default --password $1 glance >> glance.log 2>> glance-error.log
...
sudo sed -i "$[linhaauthtoken+9] i\password = $1" /etc/glance/glance-api.conf
...
sudo sed -i "$[linhaauthtoken2+9] i\password = $1" /etc/glance/glance-registry.conf
