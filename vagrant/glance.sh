...
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'localhost' IDENTIFIED BY '$2';
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'%' IDENTIFIED BY '$2';
_EOF_

source necos/vagrant/admin-openrc

openstack user create --domain default --password $2 glance >> glance.log 2>> glance-error.log
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/glance\/glance.sqlite/mysql+pymysql:\/\/glance:$2@controller\/glance/' /etc/glance/glance-api.conf
...
sudo sed -i "$[linhaauthtoken+9] i\password = $2" /etc/glance/glance-api.conf
...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/glance\/glance.sqlite/mysql+pymysql:\/\/glance:$2@controller\/glance/' /etc/glance/glance-registry.conf
...
sudo sed -i "$[linhaauthtoken2+9] i\password = $2" /etc/glance/glance-registry.conf
