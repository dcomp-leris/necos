...
sudo sed -i 's/sqlite:\/\/\/\/var\/lib\/neutron\/neutron.sqlite/mysql+pymysql:\/\/neutron:secret@controller\/neutron/' /etc/neutron/neutron.conf
...
sudo sed -i "$[linhadefaultneutron+4] i\transport_url = rabbit://openstack:secret@controller" /etc/neutron/neutron.conf
...
sudo sed -i "$[linhanovaneutron+8] i\password = secret" /etc/neutron/neutron.conf
...
sudo sed -i "$[linhavxlanneutron+2] i\local_ip = 10.0.0.11" /etc/neutron/plugins/ml2/linuxbridge_agent.ini
